...
int main(int argc, char *argv[]) {
  ...
  char s[50];
  Juu_type *Juu2L,*JuuL;
  ww_type *ww_conL,*ww_disL,*ww_con2L,*ww_dis2L; 
  params paramsL, params2L;
  ...
  for(n=N_min;n<N_max;n++){
    sprintf(s,"dati_2pointL2L/loop_SG_%d_L_2L.dat",n);
    f=fopen(s,"w");
    ...
    fprintf(f,"# 1:L 2:con_loop 3:error 4:con_loopAA 5:con_loopAB 6: con_loopAC 7:con_linea 8:error\n");
    fflush(f);
    
    Lplus1(Juu2L,ww_con2L,ww_dis2L,&params2L);
    ...
    for(i=0;i<7;i++){  
      ...
      L+=1;
      correlations(JuuL, &con, &error_con, paramsL.probJ);
      loop(L,JuuL,ww_conL,ww_disL,Juu2L,ww_con2L,ww_dis2L,paramsL,params2L,f);
      fprintf(f,"%g %g\n",con,error_con);
      fflush(f);
    }
    fclose(f);
