...
// N_min is the number that identify the first data, then 34 more data are added see line 1066
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void L1plusL2(Juu_type *Juu1, ww_type *ww_con1, ww_type *ww_dis1, params *p1, Juu_type *Juu2, ww_type *ww_con2, ww_type *ww_dis2, params *p2,Juu_type *JuuTemp, ww_type *ww_conTemp, ww_type *ww_disTemp, params *pTemp, int degree){

  int norm_probAA=0,norm_probAB=0,norm_probBA=0,check_triplet1,check_triplet2,n_triplet=0,n_couple_con=0,n_couple_dis=0,num_ABinB=0,num_BAinB=0,norm_prob_con=0;
  double A,B,C,D,J1,J2,u1,u2,ut1,ut2,h,probJ_old1,prob_con_old1,probJ_old2,prob_con_old2;

  while(n_triplet<N&&n_couple_con<N&&n_couple_dis<N){
  
    check_triplet1=extract_random_loop(Juu1, ww_con1, ww_dis1, &u1, &ut1, &J1, p1);
    check_triplet2=extract_random_loop(Juu2, ww_con2, ww_dis2, &ut2, &u2, &J2, p2);
    ...
    if(check_triplet1==2 && check_triplet2==2){ 
      if(fabs(h)>fabs(J1)+fabs(J2)){
        new_Juu[n_couple_con].u0=u1+(A+C-B-D)/4.;
        new_Juu[n_couple_con].uL=u2+(A+D-C-B)/4.;
        new_Juu[n_couple_con].J=0.;
        n_couple_con++;
        
      }else{
        new_Juu[n_triplet].u0=u1+(A+C-B-D)/4.;
        new_Juu[n_triplet].uL=u2+(A+D-C-B)/4.;
        new_Juu[n_triplet].J=(A+B-C-D)/4.;
        n_triplet++;
      }
      norm_probAA++;
    }else{
      if(check_triplet1==2 && check_triplet2==1){
        if(fabs(h)>fabs(J1)){
          new_Juu[n_couple_dis].u0=u1+(A+C-B-D)/4.;
          new_Juu[n_couple_dis].uL=u2+(A+D-C-B)/4.;
          new_Juu[n_couple_dis].J=0.;
          n_couple_dis++;
        }else{
          new_Juu[n_couple_con].u0=u1+(A+C-B-D)/4.;
          new_Juu[n_couple_con].uL=u2+(A+D-C-B)/4.;
      ...
      if(check_triplet1==1 && check_triplet2==2){
        if(fabs(h)>fabs(J2)){
          new_Juu[n_couple_dis].u0=u1+(A+C-B-D)/4.;
          new_Juu[n_couple_dis].uL=u2+(A+D-C-B)/4.;
          new_Juu[n_couple_dis].J=0.;
  ...
  while(n_triplet<N&&n_couple_con<N){
    check_triplet1=extract_triplet_or_con_loop(Juu1, ww_con1, &u1, &ut1, &J1, p1);
    check_triplet2=extract_triplet_or_con_loop(Juu2, ww_con2, &ut2, &u2, &J2, p2);
  ...
  while(n_triplet<N){
    extract_triplet_loop(Juu1, &u1, &ut1, &J1);
    extract_triplet_loop(Juu2, &ut2, &u2, &J2);
  ...
  copy_loop(JuuTemp, ww_conTemp, ww_disTemp); 

  probJ_old1=p1->probJ;             
  prob_con_old1=p1->prob_con; 
  probJ_old2=p2->probJ;             
  prob_con_old2=p2->prob_con;
  pTemp->probJ=probJ_old1*probJ_old2*(norm_probAA!=0?(double)N/(double)norm_probAA:0);  
  pTemp->prob_con = probJ_old1*probJ_old2*( 1-(norm_probAA!=0?(double)N/(double)norm_probAA:0) ) + 
                    probJ_old1*prob_con_old2*(norm_probAB!=0?(double)num_ABinB/(double)norm_probAB:0) + 
                    probJ_old2*prob_con_old1*(norm_probBA!=0?(double)num_BAinB/(double)norm_probBA:0);
  pTemp->n_con=n_couple_con;
  pTemp->n_dis=n_couple_dis;
}
...
void loop_distribution(Juu_type *JuuA, ww_type *ww_conA, ww_type *ww_disA, params *pA, Juu_type *JuuB, ww_type *ww_conB, ww_type *ww_disB, params *pB, Juu_type *JuuC, ww_type *ww_conC, ww_type *ww_disC, params *pC){
  ...
  while(n_triplet<N&&n_couple_con<N){
    ...
    if(check_triplet1==2 || check_triplet2==2){
        ...
        new_Juu[n_triplet].J=JA+JB;
        n_triplet++;
    }else if(check_triplet1==1 || check_triplet2==1){    
	      new_ww_con[n_couple_con].u0=u1A+u1B;
        new_ww_con[n_couple_con].uL=u2A+u2B;
        n_couple_con++;
    }
...
int main(int argc, char *argv[]) {
  ...
  int L=1,i,iter,n,LMAX;
  double PJ[10];
  
  FILE *f;
  char s[90];
  Juu_type *JuuL,*Juu2L,*JuuTemp1,*JuuTemp2,*JuuTemp3;
  ww_type *ww_conL,*ww_disL, *ww_con2L,*ww_dis2L,*ww_conTemp1,*ww_disTemp1,*ww_conTemp2,*ww_disTemp2,*ww_conTemp3,*ww_disTemp3; 
  params paramsL, params2L, paramsTemp1, paramsTemp2, paramsTemp3;
  ...
  ww_conTemp1=(ww_type *)malloc(N*sizeof(ww_type));
  ww_disTemp1=(ww_type *)malloc(N*sizeof(ww_type));
  ww_conTemp2=(ww_type *)malloc(N*sizeof(ww_type));
  ww_disTemp2=(ww_type *)malloc(N*sizeof(ww_type));
  ww_conTemp3=(ww_type *)malloc(N*sizeof(ww_type));
  ww_disTemp3=(ww_type *)malloc(N*sizeof(ww_type));
  ...
  JuuTemp1=(Juu_type *)malloc(N*sizeof(Juu_type));
  JuuTemp2=(Juu_type *)malloc(N*sizeof(Juu_type));
  JuuTemp3=(Juu_type *)malloc(N*sizeof(Juu_type));
  new_Juu=(Juu_type *)malloc(N*sizeof(Juu_type));

  for(n=N_min;n<N_max;n++){
    
    sprintf(s,"dati_2point_disc_LL2LL_new/disc_LL2LL_new_%d.dat",n);
    f=fopen(s,"w");
    ...
    Init_triplet(JuuTemp1,&paramsTemp1);
    Init_triplet(JuuTemp2,&paramsTemp2);
    Init_triplet(JuuTemp3,&paramsTemp3);
    
    fprintf(f,"# z: %d h: %g N: %d \n",z,field,N);
    fprintf(f,"# 1:L 2:PJ!=0 \n");
    fflush(f);
    
    Lplus1_right(Juu2L,ww_con2L,ww_dis2L,&params2L,2);

    for(LMAX=2; LMAX<11; LMAX++){
      ...
      loop_distribution(Juu2L,ww_con2L,ww_dis2L,&params2L,JuuL,ww_conL,ww_disL,&paramsL,JuuTemp1,ww_conTemp1,ww_disTemp1,&paramsTemp1);
      L1plusL2(JuuTemp1,ww_conTemp1,ww_disTemp1,&paramsTemp1,JuuL,ww_conL,ww_disL,&paramsL,JuuTemp1,ww_conTemp1,ww_disTemp1,&paramsTemp1,3);
      L1plusL2(JuuL,ww_conL,ww_disL,&paramsL,JuuTemp1,ww_conTemp1,ww_disTemp1,&paramsTemp1,JuuTemp1,ww_conTemp1,ww_disTemp1,&paramsTemp1,3);

      


      L1plusL2(JuuL,ww_conL,ww_disL,&paramsL,JuuL,ww_conL,ww_disL,&paramsL,JuuTemp2,ww_conTemp2,ww_disTemp2,&paramsTemp2,2);
      L1plusL2(JuuTemp2,ww_conTemp2,ww_disTemp2,&paramsTemp2,JuuL,ww_conL,ww_disL,&paramsL,JuuTemp2,ww_conTemp2,ww_disTemp2,&paramsTemp2,2);

      L1plusL2(Juu2L,ww_con2L,ww_dis2L,&params2L,JuuL,ww_conL,ww_disL,&paramsL,JuuTemp3,ww_conTemp3,ww_disTemp3,&paramsTemp3,2);
      L1plusL2(JuuTemp3,ww_conTemp3,ww_disTemp3,&paramsTemp3,JuuL,ww_conL,ww_disL,&paramsL,JuuTemp3,ww_conTemp3,ww_disTemp3,&paramsTemp3,2);

      fprintf(f,"%d %g\n",LMAX, - paramsTemp1.probJ + paramsTemp2.probJ + paramsTemp3.probJ);
      fflush(f);
