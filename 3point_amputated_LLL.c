...
void loopvertex(int L, Juu_type *JuuLa, ww_type *ww_conLa, ww_type *ww_disLa, Juu_type *JuuLb, ww_type *ww_conLb, ww_type *ww_disLb, Juu_type *JuuLc, ww_type *ww_conLc, ww_type *ww_disLc, params paramsLa, params paramsLb, params paramsLc, FILE *f)
{
	...
	fprintf(f, "%d %g %g %g %g %g ", L, -corr_tot, sqrt(error_corrAAA * error_corrAAA + error_corrAAB * error_corrAAB + error_corrAAC * error_corrAAC), corrAAAdouble, corrAABdouble + corrBAAdouble + corrABAdouble, corrAACdouble + corrACAdouble + corrCAAdouble);
}
...
int main(int argc, char *argv[])
{
	...
	{
		sprintf(s, "dati_3pointLLL/loopvertex_SG_%d.dat", n); 
		f = fopen(s, "w");
		...
		{ 
			...
			L += 1;
			correlations(JuuLa, &con, &error_con, paramsLa.probJ);
