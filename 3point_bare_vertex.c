...
// N_min is the number that identify the first data, then 453 more data are added see line 798
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void vertex_bare_correlations(Juu_type *Juu1, Juu_type *Juu2, Juu_type *Juu3, double *con_vertex, double probJ1, double probJ2, double probJ3)
{ 

	int i, j0, j1, j2, j3, ran1, ran2, ran3;
	double h1, h2, h3, h0, J1, J2, J3;

	(*con_vertex) = 0;

	for (i = 0; i < N; i++)
	{
		ran1 = (int)(FRANDOM * N_BP);
		ran2 = (int)(FRANDOM * N_BP);
		ran3 = (int)(FRANDOM * N_BP);

		h1 = Juu1[ran1].u0 + field;
		//  h=u2+u3+field*pm1;
		for (j1 = 0; j1 < z - 1; j1++)
		{
			h1 += u[(int)(FRANDOM * N_BP)];
		}
		h2 = Juu2[ran2].u0 + field;
		//  h=u2+u3+field*pm1;
		for (j2 = 0; j2 < z - 1; j2++)
		{
			h2 += u[(int)(FRANDOM * N_BP)];
		}
		h3 = Juu3[ran3].u0 + field;
		//  h=u2+u3+field*pm1;
		for (j3 = 0; j3 < z - 1; j3++)
		{
			h3 += u[(int)(FRANDOM * N_BP)];
		}
		h0 = Juu1[ran1].uL + Juu2[ran2].uL + Juu3[ran3].uL + field;
		//  h=u2+u3+field*pm1;
		for (j0 = 0; j0 < z - 3; j0++)
		{
		...
		J2 = Juu2[ran2].J;
		J3 = Juu3[ran3].J;
		(*con_vertex) += (double)theta(fabs(J1),fabs(h1)) * (double)theta(fabs(J2),fabs(h2)) * (double)theta(fabs(J3),fabs(h0+sign(J2)*h2+sign(J1)*h1));
	}

	// KahanSum_mean_error(data,N,con,error);
	(*con_vertex) /= (double)N;

	(*con_vertex) *= probJ1*probJ2*probJ3;
}
...
int main(int argc, char *argv[])
{
  ...
  int N_max=N_min+453;
	...
	{
		sprintf(s, "dati_barevertex/3point_barevertex_SG_%d.dat", n);
		f = fopen(s, "w");
		...
		fprintf(f, "# 1:L 2:con_vertex\n");
		fflush(f);

		L = 1;
		for (i = 0; i < 10; i++)
		{ 
			Lplus1(JuuL, ww_conL, ww_disL, &paramsL);

			L += 1;
			vertex_bare_correlations(JuuL, JuuL, JuuL, &con_vertex, paramsL.probJ, paramsL.probJ, paramsL.probJ);
			fprintf(f, "%d %g\n", L, con_vertex);
			fflush(f);
		}
		fclose(f);
